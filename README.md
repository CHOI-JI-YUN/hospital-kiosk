...
처방전 발급, 수납 대상 선택, 결제 방법 선택,
QR 결제 화면 및 영수증 출력 기능을 구현하여 실제 병원 운영 흐름을 반영했습니다.
...
### ⏳ 대기 순서 데이터 동기화 문제
- 대기열 데이터와 화면에 표시되는 순서가 일치하지 않는 문제가 발생
- Queue 자료구조를 활용하여 순서를 보장하고 처리 안정성을 확보

### 👤 환자 정보 중복 등록 문제
- 동일 환자 정보가 중복 등록되는 문제가 발생
- Dictionary 기반 조회 구조를 적용하여 중복 등록을 방지

### 🖥 UI 상태 반영 문제
- 환자 호출 시 일부 화면 정보가 즉시 갱신되지 않는 문제가 발생
- 이벤트 기반 화면 갱신 구조를 적용하여 상태 동기화를 구현
...
## 🚀 프로젝트 특징
- Queue 기반 FIFO 대기열 관리 시스템 구현
- Dictionary 기반 환자 정보 조회 및 관리 기능 구현
- 병원 접수부터 수납까지 실제 운영 흐름을 반영한 시스템 구현
- QR 결제 및 영수증 출력 기능 구현
- 사용자 입력에 따라 동적으로 변화하는 UI 구성

---

## 📈 구현 결과 및 효과
- 환자 접수부터 대기, 호출, 처방전 발급, 수납, 영수증 출력까지 하나의 흐름으로 통합 구현
...
## 🙋 담당 역할
- 개인 프로젝트 전체 설계 및 개발
- WinForms 기반 병원 키오스크 UI 구현
- Queue 기반 대기열 관리 시스템 구현
- Dictionary 기반 환자 정보 조회 기능 구현
- 처방전 발급 기능 구현
- 수납 및 결제 처리 기능 구현
- QR 결제 시뮬레이션 및 영수증 출력 기능 구현
