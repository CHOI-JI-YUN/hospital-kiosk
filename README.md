...
## ⚙️ 문제 해결 경험
- 대기 순서 관리 시 데이터 동기화 문제 발생  
  → Queue 자료구조를 활용하여 순서 보장 및 처리 안정성 확보  

- 기존 환자 조회 시 중복 데이터 처리 문제  
  → Dictionary를 활용하여 빠른 검색 및 데이터 중복 방지 구현  

- UI 상태와 데이터 불일치 문제  
  → 이벤트 기반 흐름 제어로 상태 동기화 해결
...
- Queue 기반 대기열 관리로 공정한 순서 처리 가능  
- Dictionary를 활용한 환자 조회 속도 향상  
